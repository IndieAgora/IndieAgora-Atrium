...
function ia_message_render_threads(array $rows, int $me_phpbb): array {
  ...
  foreach ($rows as $t) {
    ...
    $row_title = isset($t['title']) ? trim((string)$t['title']) : '';
    $row_avatar = isset($t['avatar_url']) ? trim((string)$t['avatar_url']) : '';

    // DM convenience meta (for profile deep-links)
    $dm_other_id = 0;
    $dm_other_username = "";

    // Avatar: prefer stored thread avatar (group threads), else DM avatar.
    $avatar = $row_avatar;
    ...
    $out[] = [
      ...
      'title' => ($row_title !== '') ? $row_title : ia_message_thread_title_from_key($thread_key, $me_phpbb),
      'avatarUrl' => $avatar,
...
function ia_message_thread_title_from_key(string $key, int $me_phpbb): string {
  if (strpos($key, 'dm:') === 0) {
    ...
    if (count($parts) === 3) {
      ...
      return function_exists('ia_message_display_ui_name_for_phpbb_id') ? ia_message_display_ui_name_for_phpbb_id($other) : ia_message_display_label_for_phpbb_id($other);
    }
  }
  if (strpos($key, 'grp:') === 0) return 'Group';
  return 'Thread';
}