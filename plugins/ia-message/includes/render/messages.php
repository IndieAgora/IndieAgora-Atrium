...
function ia_message_render_messages(array $rows, int $me_phpbb): array {
  ...
  if (!empty($reply_ids)) {
    ...
    foreach ((array)$rrows as $r) {
      ...
      $reply_map[$id] = [
        ...
        'author_label' => function_exists('ia_message_display_ui_name_for_phpbb_id') ? ia_message_display_ui_name_for_phpbb_id($author) : (function_exists('ia_message_display_label_for_phpbb_id') ? ia_message_display_label_for_phpbb_id($author) : ('User #' . $author)),
        'excerpt' => $plain,
  ...
  foreach ($rows as $m) {
    ...
    $out[] = [
      ...
      'author_phpbb_user_id' => $author,
      'author_label' => function_exists('ia_message_display_ui_name_for_phpbb_id') ? ia_message_display_ui_name_for_phpbb_id($author) : (function_exists('ia_message_display_label_for_phpbb_id') ? ia_message_display_label_for_phpbb_id($author) : ('User #' . $author)),
      'author_avatarUrl' => function_exists('ia_message_avatar_url_for_phpbb_id') ? (string) ia_message_avatar_url_for_phpbb_id($author, 48) : '',
