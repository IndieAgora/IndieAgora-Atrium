...
class IA_PT_PeerTube_Mint {
    ...
    private static function password_grant(string $base, array $client, string $username, string $password): array {
        ia_pt_trace_log('ia-pt-mint.password_grant.begin', ['identifier' => $username, 'context' => 'token_mint']);
        ...
        if (is_wp_error($resp)) {
            ia_pt_trace_log('ia-pt-mint.password_grant.fail', ['identifier' => $username, 'http' => 0, 'message' => $resp->get_error_message()]);
        ...
        if ($code >= 200 && $code < 300 && is_array($json) && !empty($json['access_token'])) {
            ia_pt_trace_log('ia-pt-mint.password_grant.ok', ['identifier' => $username, 'http' => $code]);
        ...
        ia_pt_trace_log('ia-pt-mint.password_grant.fail', ['identifier' => $username, 'http' => $code, 'message' => $msg]);
