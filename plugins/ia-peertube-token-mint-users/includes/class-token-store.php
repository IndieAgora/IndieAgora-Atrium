...
class IA_PeerTube_Token_Store {
    ...
    public static function import_legacy_row(int $phpbb_user_id, array $legacy_row): bool {
        global $wpdb;
        $now = current_time('mysql', 1);
        ...
        $ok = $wpdb->replace(IA_PT_TOKENS_TABLE, [
            ...
            'access_token_enc'   => (string)($legacy_row['access_token_enc'] ?? ''),
            'refresh_token_enc'  => (string)($legacy_row['refresh_token_enc'] ?? ''),
            'expires_at'         => !empty($legacy_row['expires_at_utc']) ? (string)$legacy_row['expires_at_utc'] : null,
            'last_refresh_at'    => $now,
            'last_mint_at'       => null,
            'last_mint_error'    => null,
            'created_at'         => $now,
        ...
        if ($ok === false && !empty($wpdb->last_error)) {
            self::touch_mint_error($phpbb_user_id, 'Legacy token import failed: ' . $wpdb->last_error);
            return false;
        }
        return true;
    }
     ...
     */
    public static function update_access(int $phpbb_user_id, string $access_enc, ?string $expires_at, ?string $refresh_enc = null): bool {
        global $wpdb;
        $now = current_time('mysql', 1);
        ...
        // Some OAuth servers rotate refresh tokens on refresh. If PeerTube returns a new refresh_token, persist it.
        if ($refresh_enc !== null && $refresh_enc !== '') {
            $data['refresh_token_enc'] = $refresh_enc;
        }
        ...
        if ($ok === 0) {
            if ($refresh_enc !== null && $refresh_enc !== '') {
                $q = $wpdb->prepare(
                    "INSERT INTO " . IA_PT_TOKENS_TABLE . " (phpbb_user_id, access_token_enc, refresh_token_enc, expires_at, last_refresh_at, created_at, updated_at)
                     VALUES (%d, %s, %s, %s, %s, %s, %s)
                     ON DUPLICATE KEY UPDATE access_token_enc=VALUES(access_token_enc), refresh_token_enc=VALUES(refresh_token_enc), expires_at=VALUES(expires_at), last_refresh_at=VALUES(last_refresh_at), updated_at=VALUES(updated_at)",
                    $phpbb_user_id, $access_enc, $refresh_enc, $expires_at, $now, $now, $now
                );
            } else {
                $q = $wpdb->prepare(
