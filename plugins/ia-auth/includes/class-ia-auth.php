...
final class IA_Auth {
    ...
    public function handle_routes() {
        ...
        if ($route === 'register') {
            ...
            echo '<form class="ia-auth-form" data-action="ia_user_login">';
            echo '<input type="hidden" name="nonce" value="' . esc_attr(wp_create_nonce('ia_auth_nonce')) . '">';
    ...
    public function ajax_login() {
        // Canonical login ladder lives on ia_user_login. Keep ia_auth_login as a compatibility shim
        // so older forms/routes do not trigger a separate auth braid.
        if (class_exists('IA_User_PeerTube_Fallback_Clean') && method_exists('IA_User_PeerTube_Fallback_Clean', 'ajax_login')) {
            ia_pt_trace_log('ia-auth.ajax_login.delegate', [
    ...
    ia_pt_trace_log('ia-auth.ajax_login.enter', [
        'identifier' => (string)($_POST['identifier'] ?? ''),
        'uri' => (string)($_SERVER['REQUEST_URI'] ?? ''),
    ]);
    ...
    // Deleted accounts are hard-blocked from logging in again. They must re-register.
    if (function_exists('ia_goodbye_identifier_is_tombstoned') && ia_goodbye_identifier_is_tombstoned($id)) {
        wp_send_json_error(['message' => 'This account was deleted. Please register again to rejoin.'], 403);
    }

    $engine = self::engine_normalized();

    // Prefer phpBB auth when available, but do not hard-depend on it.
    // After phpBB removal or outage, native WP-authenticated Atrium accounts must still be able to log in.
    $phpbb_user = [];
    $wp_user_id = 0;
    $did_wp_signon = false;
    $phpbb_auth_message = 'Login failed.';

    $phpbb = (array)($engine['phpbb'] ?? []);
    $phpbb_configured = !empty($phpbb['host']) && !empty($phpbb['name']) && !empty($phpbb['user']);

    if ($phpbb_configured) {
        $auth = $this->phpbb->authenticate($id, $pw, $phpbb);
        if (!empty($auth['ok'])) {
    ...
    // Fallback: try native WP auth for WP-backed Atrium accounts.
    if ($wp_user_id <= 0) {
        $login = $id;
        if (is_email($id)) {
        ...
        if (is_wp_error($user) || empty($user->ID)) {
            ...
            $probe = null;
            if (is_email($id)) {
                $probe = get_user_by('email', $id);
            }
            if ((!$probe || is_wp_error($probe)) && $login !== '') {
                $probe = get_user_by('login', $login);
            }
        ...
        if ($phpbb_user_id > 0) {
            $phpbb_user = [
                ...
                'username_clean' => (string)$user->user_login,
    ...
    if (!$did_wp_signon) {
        $uobj = get_user_by('id', (int)$wp_user_id);
    ...
    // If the account was deactivated in phpBB, treat a successful login as reactivation.
    if ($phpbb_user_id > 0) {
        try {
            $this->phpbb->reactivate_if_deactivated($phpbb_user_id);
            delete_user_meta((int)$wp_user_id, 'ia_deactivated');
            wp_update_user(['ID' => (int)$wp_user_id, 'user_status' => 0]);
        } catch (Throwable $e) {
            // ignore
        }
    }
    ...
    if (($opt['peertube_oauth_method'] ?? 'password_grant') === 'password_grant' && $phpbb_user_id > 0) {
        ...
        ia_pt_trace_log('ia-auth.ajax_login.direct_token_attempt', ['identifier' => $id, 'phpbb_user_id' => $phpbb_user_id]);
        ...
        if (empty($pt['ok'])) {
            ...
            foreach ($cands as $cand) {
                ia_pt_trace_log('ia-auth.ajax_login.candidate_token_attempt', ['identifier' => (string)$cand, 'source' => 'candidate_loop']);
