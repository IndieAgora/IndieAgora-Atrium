...
final class IA_User {
    ...
    private function __construct() {
        ...
        if (is_admin() && class_exists('IA_User_Admin')) {
            new IA_User_Admin($this);
        }
    }
...
do_action('ia_user_after_register', $phpbb_user, $wp_user_id);
    ...
    public function phpbb_user_display_name(int $phpbb_user_id): string {
        ...
        try {
            if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $map)) === $map) {
                $wp_user_id = (int) $wpdb->get_var($wpdb->prepare(
                    "SELECT wp_user_id FROM {$map} WHERE phpbb_user_id = %d LIMIT 1",
                    $phpbb_user_id
                ));
            }
        } catch (Throwable $e) {
        ...
        if ($wp_user_id <= 0) {
            $keys = ['ia_phpbb_user_id', 'phpbb_user_id', 'ia_phpbb_uid', 'phpbb_uid', 'ia_identity_phpbb'];
            foreach ($keys as $key) {
        ...
        if ($wp_user_id > 0) {
            ...
            if ($user) {
                ...
                $nickname = trim((string) get_user_meta($wp_user_id, 'nickname', true));
