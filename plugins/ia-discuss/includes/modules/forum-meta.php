...
final class IA_Discuss_Module_Forum_Meta implements IA_Discuss_Module_Interface {
  ...
  public function ajax_forum_meta(): void {
    ...
    // Agora view should show the full description (not an excerpt),
    // while the Agoras list keeps its own excerpt rendering.
    $desc_html = $this->bbcode->format_post_html((string)($row['forum_desc'] ?? ''));
    $rules_raw = (string)($row['forum_rules'] ?? '');
    $rules_html = $rules_raw !== '' ? $this->bbcode->format_post_html($rules_raw) : '';
    ...
    ia_discuss_json_ok([
      ...
      'forum_rules_html' => $rules_html,
      'has_rules' => $rules_raw !== '' ? 1 : 0,
