  ...
  function renderFeedInto(mount, view, forumId) {
    ...
    loadFeed(view, forumId).then((data) => {
      ...
      function openTopicFromEvent(e, scroll) {
        ...
        if (!card) return;
        const tid = parseInt(card.getAttribute("data-topic-id") || "0", 10);
        if (!tid) return;
      ...
      mount.querySelectorAll("[data-open-agora]").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          e.preventDefault();
          e.stopPropagation();
          const fid = parseInt(btn.getAttribute("data-forum-id") || "0", 10);
          if (!fid) return;
      ...
      mount.querySelectorAll("[data-iad-open-links]").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          const card = btn.closest("[data-topic-id]");
          const title = card ? (card.querySelector(".iad-card-title")?.textContent || "") : "";
      ...
      mount.querySelectorAll("[data-iad-open-video]").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          const card = btn.closest("[data-topic-id]");
          const title = card ? (card.querySelector(".iad-card-title")?.textContent || "") : "Video";
      ...
      mount.querySelectorAll("[data-share-topic]").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          const title = card.querySelector(".iad-card-title")?.textContent || "";
          const url = makeTopicUrl(tid);
