...
function setBellState(root, enabled){
  ...
  function redirectToGlobalFeeds(){
    try {
      const u = new URL(window.location.href);
      u.searchParams.set('tab', 'discuss');
      u.searchParams.set('ia_tab', 'discuss');
      u.searchParams.set('iad_view', 'new');
      u.searchParams.delete('iad_forum');
      u.searchParams.delete('topic_id');
      window.history.pushState(window.history.state || {}, '', u.toString());
    } catch (e) {}
    try {
      if (window.IA_Discuss && window.IA_Discuss.router && typeof window.IA_Discuss.router.render === 'function') {
        const root = document.querySelector('.iad-page') || document.querySelector('[data-iad-root]') || document.body;
        window.IA_Discuss.router.render('new', 0, 'New');
        return;
      }
    } catch (e) {}
    try { window.dispatchEvent(new CustomEvent('iad:go_feed', { detail: { tab: 'new' } })); } catch (e) {}
  }
  ...
  async function toggleJoin(forumId, root){
    ...
    if (joined && String(data.access_revoked || '0') === '1') {
      redirectToGlobalFeeds();
    }
  }
