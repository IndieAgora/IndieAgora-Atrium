(function () {
  ...
  function isAudioMime(m) { return /^audio\//i.test(String(m || "")); }

  function isLikelyAudioUrl(url) {
    const u = String(url || "").trim();
    if (!u) return false;
    const lu = u.toLowerCase();
    if (/\.(mp3|m4a|aac|wav|wave|flac|oga|ogg|opus|weba)(\?|$)/i.test(lu)) return true;
    return false;
  }
  ...
  function isLikelyVideoUrl(url) {
    ...
    if (lu.includes("youtube.com/shorts/")) return true;
    if (lu.includes("youtube.com/live/")) return true;
  ...
  function buildVideoEmbedHtml(url) {
    ...
    const yt = parseYouTubeMeta(raw);
    if (yt && (yt.id || yt.isPlaylist)) {
      const YT = window.IA_DISCUSS_YOUTUBE || {};
      const embed = YT.buildEmbed ? YT.buildEmbed(yt) : "";
      return `
        <div class="iad-att-media">
          <iframe class="iad-att-iframe${yt.isShort ? ' is-vertical' : ''}" src="${esc(embed)}" title="Video"
  ...
  function inlineMediaHTML(media) {
    ...
    if (atts && atts.length) {
      ...
      // Audio attachments render as Agora Player blocks
      const auds = atts.filter((a) => isAudioMime(a && a.mime) || isLikelyAudioUrl(a && a.url));
      auds.slice(0, 3).forEach((au) => {
        if (!au || !au.url) return;
        blocks.push(agoraPlayerHTML(au));
      });
