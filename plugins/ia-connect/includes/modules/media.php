...
final class ia_connect_module_media implements ia_connect_module_interface {
  ...
  private function upload(string $field, string $meta_key, string $size): void {
    ...
    // Store a legacy URL meta for compatibility with older builds.
    if ($field === 'avatar') {
      update_user_meta($user_id, 'ia_connect_avatar_url', $url ?: '');
    } else {
      update_user_meta($user_id, 'ia_connect_cover_url', $url ?: '');
    }
