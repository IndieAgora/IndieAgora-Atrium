...
function ia_connect_send_email(int $to_wp_user_id, string $key, array $ctx, string $fallback_subject, string $fallback_body): void {
  ...
  if (function_exists('ia_notify_emails_enabled_for_wp') && !ia_notify_emails_enabled_for_wp($to_wp_user_id)) {
    return;
  }
...
function ia_connect_create_mention_posts(int $original_post_id, int $actor_wp_id, array $mentioned_wp_ids, int $comment_id = 0): void {
  ...
  foreach ($mentioned_wp_ids as $wp_id) {
    ...
    $wpdb->insert($posts, [
      ...
      'shared_tab' => '',
      'shared_ref' => ('mention:' . (int)$comment_id),
      'title' => '',
      'body' => '',
      'created_at' => $now,
      'updated_at' => $now,
...
function ia_connect_notify_on_comment_created(int $comment_id, int $post_id, int $actor_wp_id): void {
  ...
  // Followers (explicit follow/unfollow)
$watchers = ia_connect_followers_wp_ids($post_id);
// Safety: ensure wall owner and post author are followed unless they explicitly unfollowed.
// (They are auto-followed on create/reply, but older posts may predate follows.)
