...
function ia_stream_ajax_video_rate(): void {
  ...
  $status = ['ok' => false, 'code' => 'missing_user_token', 'error' => 'missing_user_token', 'token' => ''];
  try {
  ...
  if (empty($status['ok']) || $token === '') {
    wp_send_json([
      'ok' => false,
      'code' => (string)($status['code'] ?? 'missing_user_token'),
      'error' => (string)($status['error'] ?? 'missing_user_token'),
      'token_source' => (string)($status['token_source'] ?? 'ia_peertube_user_tokens'),
    ]);
  }
  ...
  try {
    if (method_exists('IA_PeerTube_Token_Helper', 'get_token_status_for_current_user')) {
      ...
      wp_send_json([
        ...
        'error' => (string)($status['error'] ?? 'Mint failed'),
        'token_source' => (string)($status['token_source'] ?? 'ia_peertube_user_tokens'),
      ]);
    }
