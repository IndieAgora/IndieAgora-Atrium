...
final class IA_Stream_Module_Comments implements IA_Stream_Module_Interface {
  ...
  public static function create_thread(array $q): array {
    ...
    $status = ['ok' => false, 'code' => 'missing_user_token', 'error' => 'PeerTube token missing for this account.', 'token' => ''];
    if (class_exists('IA_PeerTube_Token_Helper') && method_exists('IA_PeerTube_Token_Helper', 'get_token_status_for_current_user')) {
      try {
        ...
        $status = ['ok' => false, 'code' => 'token_helper_exception', 'error' => $e->getMessage(), 'token' => ''];
      }
    } elseif (class_exists('IA_PeerTube_Token_Helper') && method_exists('IA_PeerTube_Token_Helper', 'get_token_for_current_user')) {
      try {
        $status['token'] = (string) IA_PeerTube_Token_Helper::get_token_for_current_user();
        $status['ok'] = trim((string)$status['token']) !== '';
        if (!empty($status['ok'])) $status['code'] = 'valid_token';
      } catch (Throwable $e) {
        $status = ['ok' => false, 'code' => 'token_helper_exception', 'error' => $e->getMessage(), 'token' => ''];
      }
    }
    ...
    if (empty($status['ok']) || $tok === '') {
      return [
        'ok' => false,
        'error' => (string)($status['error'] ?? 'PeerTube token missing for this account.'),
